...
is_page_allocatable(size_t pgnum) {
  ...
  if (!mmap_base || !mmap_end)
    return true; //Assume page is allocatabale if no loading parameters were passed.
...
boot_alloc(uint32_t n) {
  ...
  //This isn't the best algorithm, but it's better, than all that I found elsewhere.
  //It assumes that after "end" we have a contigious block of free pages and a few pages used by UEFI right before the end of mapped memory.
  //I checked that on my qemu, that assumption was true, howewer it's not guaranteed to be true in all cases (or I haven't found such guarantee in docs).
  //I hope I won't have to write absolultely correct algorithm here in future. It seems, that correct algorithm will be overcomplicated.
  const uint64_t MAX_PHYS_PAGE = 0x40000;
  uint64_t mem_needed = ROUNDUP(n, PGSIZE);
  uint64_t pages_needed = mem_needed / PGSIZE;

  uint64_t page_start = PADDR(nextfree) / PGSIZE;
  uint64_t pages_avalaible = 0;
  while(pages_avalaible < pages_needed && page_start + pages_avalaible < MAX_PHYS_PAGE) {
    if (is_page_allocatable(page_start + pages_avalaible)) {
      pages_avalaible++;
    } else {
      panic("Out of memory!");
    }
  }
  if (pages_avalaible == pages_needed) {
    char *result = nextfree;
    nextfree += pages_avalaible * PGSIZE;
    return result;
  } else {
    panic("Out of memory");
  }
...
mem_init(void) {
  ...
  // Permissions: kernel RW, user NONE
  // LAB 7: 2^32 - KERNBASE - не актуально, у нас KERNBASE больше, видимо осталось от 32-битного JOS
...
page_init(void) {
  ...
  //  2) The rest of base memory, [PGSIZE, npages_basemem * PGSIZE)
  //     is free. Assumption: is_page_allocatable(1) == true
  pages[1].pp_ref = 0;
...
pml4e_walk(pml4e_t *pml4e, const void *va, int create) {
  // LAB 7:
  uint16_t pml4_ind = PML4(va);
  if (pml4e[pml4_ind] & PTE_P) {
    return pdpe_walk((pdpe_t *)KADDR(PTE_ADDR(pml4e[pml4_ind])), va, create); 
  }
  if (create) {
    struct PageInfo *np;
    ...
    if (np) {
      np->pp_ref++;
      pml4e[pml4_ind] = page2pa(np) | PTE_P | PTE_U | PTE_W;
      return pdpe_walk((pdpe_t *)KADDR(PTE_ADDR(pml4e[pml4_ind])), va, create);
    }
  }
  return NULL;
...
pdpe_walk(pdpe_t *pdpe, const void *va, int create) {
  // LAB 7:
  uint16_t pdpt_ind = PDPE(va);
  if (pdpe[pdpt_ind] & PTE_P) {
    return pgdir_walk((pde_t *)KADDR(PTE_ADDR(pdpe[pdpt_ind])), va, create); 
  }
  if (create) {
    struct PageInfo *np;
    np = page_alloc(ALLOC_ZERO);
    if (np) {
      np->pp_ref++;
      pdpe[pdpt_ind] = page2pa(np) | PTE_P | PTE_U | PTE_W;
      return pgdir_walk((pde_t *)KADDR(PTE_ADDR(pdpe[pdpt_ind])), va, create);
    }
  }
  return NULL;
...
pgdir_walk(pde_t *pgdir, const void *va, int create) {
  // LAB 7: 
  uint16_t pgdir_ind = PDX(va);
  if (pgdir[pgdir_ind] & PTE_P) {
    return (pde_t *)KADDR(PTE_ADDR(pgdir[pgdir_ind])) + PTX(va); 
  }
  if (create) {
    struct PageInfo *np;
    np = page_alloc(ALLOC_ZERO);
    if (np) {
      np->pp_ref++;
      pgdir[pgdir_ind] = page2pa(np) | PTE_P | PTE_U | PTE_W;
      return (pde_t *)KADDR(PTE_ADDR(pgdir[pgdir_ind])) + PTX(va);
    }
  }
  return NULL;
...
boot_map_region(pml4e_t *pml4e, uintptr_t va, size_t size, physaddr_t pa, int perm) {
  ...
  for (size_t i = 0; i < size; i += PGSIZE) {
    pte_t *pte = pml4e_walk(pml4e, (void *)(va + i), 1);
    if (pte) {
      *pte = (pa + i) | perm | PTE_P;
    } else {
      panic("boot_map_region: Failed to locate page table.");
    }
  }
}
...
page_insert(pml4e_t *pml4e, struct PageInfo *pp, void *va, int perm) {
  ...
  pp->pp_ref++; //increase ref count to avoid the corner case that pp is freed before it is inserted.
  if (*pte & PTE_P) {
...
page_remove(pml4e_t *pml4e, void *va) {
  // LAB 7:
  pte_t *pte;
  struct PageInfo * pg = page_lookup(pml4e, va, &pte);
  if (pg) {
    *pte = 0;
    page_decref(pg);
